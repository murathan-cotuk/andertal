...
export function getAutomationsCopy(locale) {
  const t = (en, tr, fr, es, it, de) => lt(locale, en, tr, fr, es, it, de);
  const suffixDays = t("days", "gün", "jours", "días", "giorni", "Tage");
  const suffixPcs = t("pcs", "adet", "pièces", "uds.", "pz.", "Stück");
  const suffixOrders = t("orders", "sipariş", "commandes", "pedidos", "ordini", "Bestellungen");
  const emailSubject = t("Email subject", "E-posta konusu", "Objet de l'e-mail", "Asunto del correo", "Oggetto email", "E-Mail-Betreff");
  const discount = t("Discount", "İndirim", "Remise", "Descuento", "Sconto", "Rabatt");

  const categories = [
    { id: "retention", color: "#3b82f6", icon: "💙", label: t("Customer retention", "Müşteri bağlılığı", "Fidélisation client", "Fidelización de clientes", "Fidelizzazione clienti", "Kundenbindung") },
    { id: "inventory", color: "#f97316", icon: "🟠", label: t("Inventory & operations", "Stok & operasyon", "Stock & opérations", "Inventario y operaciones", "Magazzino e operazioni", "Lager & Betrieb") },
    { id: "revenue", color: "#10b981", icon: "💚", label: t("Revenue", "Gelir", "Chiffre d'affaires", "Ingresos", "Fatturato", "Umsatz") },
  ];
  ...
  const automationDefs = [
    {
      ...
      accentColor: "#f59e0b",
      accentBg: "#fef3c7",
      title: t("Review request", "Değerlendirme isteği", "Demande d'avis", "Solicitud de reseña", "Richiesta recensione", "Bewertungsanfrage"),
      desc: t("Automatically asks customers for a product review after delivery — the strongest lever for social proof.", "Teslimattan sonra müşterilerden otomatik ürün değerlendirmesi ister — sosyal kanıt için en güçlü kaldıraç.", "Demande automatiquement un avis produit après livraison — le levier le plus fort pour la preuve sociale.", "Pide automáticamente una reseña del producto tras la entrega — la palanca más fuerte para prueba social.", "Chiede automaticamente una recensione prodotto dopo la consegna — la leva più forte per la prova sociale.", "Bittet Kunden nach der Lieferung automatisch um eine Produktbewertung — der stärkste Hebel für Social Proof."),
      trigger: t("Delivery status \"delivered\"", "Teslimat durumu \"teslim edildi\"", "Statut de livraison « livré »", "Estado de entrega «entregado»", "Stato consegna «consegnato»", "Lieferstatus „zugestellt“"),
      fields: [
        { key: "delay_days", label: t("Delay after delivery", "Teslimattan sonra gecikme", "Délai après livraison", "Retraso tras entrega", "Ritardo dopo consegna", "Verzögerung nach Lieferung"), type: "number", suffix: suffixDays, default: 3, min: 1, max: 14 },
        { key: "email_subject", label: emailSubject, type: "text", default: t("How was your order? Your opinion matters!", "Siparişiniz nasıldı? Görüşünüz önemli!", "Comment s'est passée votre commande ? Votre avis compte !", "¿Cómo fue tu pedido? ¡Tu opinión importa!", "Com'è andato il tuo ordine? La tua opinione conta!", "Wie war Ihre Bestellung? Ihre Meinung zählt!") },
        { key: "email_body", label: t("Email body (optional)", "E-posta metni (isteğe bağlı)", "Corps de l'e-mail (optionnel)", "Cuerpo del correo (opcional)", "Corpo email (opzionale)", "E-Mail-Text (optional)"), type: "textarea", default: "" },
      ],
      stat_label: t("emails sent", "e-posta gönderildi", "e-mails envoyés", "correos enviados", "email inviate", "E-Mails versendet"),
      status: "live",
    },
    {
      id: "welcome_email",
      ...
      accentColor: "#3b82f6",
      accentBg: "#dbeafe",
      title: t("Welcome email", "Hoş geldin e-postası", "E-mail de bienvenue", "Correo de bienvenida", "Email di benvenuto", "Willkommens-E-Mail"),
      desc: t("Welcomes new customers personally after their first order. Proven to increase repeat purchase rate by up to 30%.", "İlk siparişten sonra yeni müşterileri kişisel olarak karşılar. Tekrar satın alma oranını %30'a kadar artırır.", "Accueille personnellement les nouveaux clients après leur première commande. Augmente le taux de réachat jusqu'à 30 %.", "Da la bienvenida personalmente a nuevos clientes tras su primer pedido. Aumenta la tasa de recompra hasta un 30 %.", "Accoglie personalmente i nuovi clienti dopo il primo ordine. Aumenta il tasso di riacquisto fino al 30%.", "Begrüßt neue Kunden nach ihrer ersten Bestellung persönlich. Erhöht Wiederkaufrate nachweislich um bis zu 30 %."),
      trigger: t("Customer's first order", "Müşterinin ilk siparişi", "Première commande d'un client", "Primer pedido de un cliente", "Primo ordine di un cliente", "Erste Bestellung eines Kunden"),
      fields: [
        { key: "email_subject", label: emailSubject, type: "text", default: t("Welcome — thank you for your trust!", "Hoş geldiniz — güveniniz için teşekkürler!", "Bienvenue — merci pour votre confiance !", "¡Bienvenido — gracias por tu confianza!", "Benvenuto — grazie per la fiducia!", "Willkommen — danke für Ihr Vertrauen!") },
        { key: "include_discount", label: t("Include discount code for repeat purchase", "Tekrar alışveriş için indirim kodu ekle", "Inclure un code promo pour un second achat", "Incluir código de descuento para segunda compra", "Includi codice sconto per secondo acquisto", "Rabattcode für Zweitkauf beilegen"), type: "toggle", default: false },
        { key: "discount_pct", label: discount, type: "number", suffix: "%", default: 10, min: 5, max: 30, depends_on: "include_discount" },
      ],
      stat_label: t("welcome emails", "hoş geldin e-postası", "e-mails de bienvenue", "correos de bienvenida", "email di benvenuto", "Willkommensmails"),
      status: "live",
    },
    {
      id: "reorder_reminder",
      ...
      accentColor: "#8b5cf6",
      accentBg: "#ede9fe",
      title: t("Reorder reminder", "Yeniden sipariş hatırlatıcısı", "Rappel de réachat", "Recordatorio de recompra", "Promemoria riordino", "Nachkauf-Erinnerung"),
      desc: t("Reminds returning customers after a defined period to reorder products.", "Belirli bir süre sonra mevcut müşterilere ürünleri yeniden sipariş etmelerini hatırlatır.", "Rappelle aux clients fidèles après un délai défini de recommander des produits.", "Recuerda a clientes habituales tras un periodo definido que reordenen productos.", "Ricorda ai clienti abituali dopo un periodo definito di riordinare i prodotti.", "Erinnert Bestandskunden nach einer definierten Zeit daran, Produkte nachzubestellen."),
      trigger: t("X days after last order", "Son siparişten X gün sonra", "X jours après la dernière commande", "X días después del último pedido", "X giorni dopo l'ultimo ordine", "X Tage nach letzter Bestellung"),
      fields: [
        { key: "delay_days", label: t("Days after last order", "Son siparişten sonra gün", "Jours après la dernière commande", "Días después del último pedido", "Giorni dopo l'ultimo ordine", "Tage nach letzter Bestellung"), type: "number", suffix: suffixDays, default: 30, min: 7, max: 180 },
        { key: "email_subject", label: emailSubject, type: "text", default: t("Time to reorder?", "Yeniden sipariş zamanı mı?", "Il est temps de recommander ?", "¿Hora de volver a pedir?", "È ora di riordinare?", "Zeit zum Nachbestellen?") },
      ],
      stat_label: t("reminders", "hatırlatma", "rappels", "recordatorios", "promemoria", "Erinnerungen"),
      status: "live",
    },
    {
      id: "abandoned_cart",
      ...
      accentColor: "#ec4899",
      accentBg: "#fce7f3",
      title: t("Abandoned cart", "Terk edilmiş sepet", "Panier abandonné", "Carrito abandonado", "Carrello abbandonato", "Warenkorbabbruch"),
      desc: t("Re-engages customers who left checkout. One of the most effective recovery flows in e-commerce.", "Ödemeyi terk eden müşterileri yeniden kazanır. E-ticarette en etkili kurtarma akışlarından biri.", "Réactive les clients ayant quitté le paiement. L'un des flux de récupération les plus efficaces.", "Reactiva clientes que abandonaron el pago. Uno de los flujos de recuperación más efectivos.", "Riattiva clienti che hanno abbandonato il checkout. Uno dei flussi di recupero più efficaci.", "Reaktiviert Kunden, die den Checkout verlassen haben. Einer der effektivsten Recovery-Flows im E-Commerce."),
      trigger: t("Cart abandoned without purchase", "Satın alma olmadan sepet terk edildi", "Panier abandonné sans achat", "Carrito abandonado sin compra", "Carrello abbandonato senza acquisto", "Warenkorb verlassen ohne Kauf"),
      fields: [
        { key: "delay_hours", label: t("Hours until email", "E-postaya kadar saat", "Heures avant l'e-mail", "Horas hasta el correo", "Ore fino all'email", "Stunden bis zur E-Mail"), type: "number", suffix: "h", default: 4, min: 1, max: 72 },
        { key: "include_discount", label: t("Offer discount code", "İndirim kodu sun", "Proposer un code promo", "Ofrecer código de descuento", "Offri codice sconto", "Rabattcode anbieten"), type: "toggle", default: true },
        { key: "discount_pct", label: discount, type: "number", suffix: "%", default: 5, min: 5, max: 20, depends_on: "include_discount" },
      ],
      stat_label: t("Recovered", "Kurtarıldı", "Récupérés", "Recuperados", "Recuperati", "Recovered"),
      status: "live",
    },
    {
      id: "low_stock_alert",
      ...
      accentColor: "#f97316",
      accentBg: "#ffedd5",
      title: t("Low stock alert", "Düşük stok uyarısı", "Alerte stock bas", "Alerta de stock bajo", "Avviso scorte basse", "Lagerbestand-Warnung"),
      desc: t("Notifies you automatically when product stock falls below the threshold. Never run out without warning again.", "Ürün stoğu eşiğin altına düştüğünde otomatik bildirir. Bir daha uyarısız tükenmeyin.", "Vous notifie automatiquement quand le stock d'un produit passe sous le seuil. Plus jamais en rupture sans avertissement.", "Te notifica automáticamente cuando el stock cae bajo el umbral. Nunca te quedes sin stock sin aviso.", "Ti avvisa automaticamente quando lo stock scende sotto la soglia. Mai più esaurito senza preavviso.", "Benachrichtigt dich automatisch, wenn der Bestand eines Produkts unter den Schwellenwert fällt. Nie wieder ausverkauft ohne Vorwarnung."),
      trigger: t("Stock < threshold", "Stok < eşik", "Stock < seuil", "Stock < umbral", "Stock < soglia", "Lagerbestand < Schwellenwert"),
      fields: [
        { key: "threshold", label: t("Threshold", "Eşik", "Seuil", "Umbral", "Soglia", "Schwellenwert"), type: "number", suffix: suffixPcs, default: 5, min: 1, max: 500 },
        { key: "alert_email", label: t("Notification email", "Bildirim e-postası", "E-mail de notification", "Correo de notificación", "Email di notifica", "Benachrichtigungs-E-Mail"), type: "email", default: "" },
      ],
      stat_label: t("alerts sent", "uyarı gönderildi", "alertes envoyées", "alertas enviadas", "avvisi inviati", "Warnungen gesendet"),
      status: "live",
    },
    {
      id: "loyalty_reward",
      ...
      accentColor: "#10b981",
      accentBg: "#d1fae5",
      title: t("Loyalty reward", "Sadakat ödülü", "Récompense fidélité", "Recompensa de fidelidad", "Premio fedeltà", "Treuebelohnung"),
      desc: t("Rewards loyal repeat customers with an automatic discount code after a set number of orders.", "Belirli sipariş sayısından sonra sadık müşterilere otomatik indirim kodu verir.", "Récompense les clients fidèles avec un code promo automatique après un nombre de commandes défini.", "Recompensa a clientes fieles con un código de descuento automático tras un número de pedidos.", "Premia i clienti fedeli con un codice sconto automatico dopo un numero di ordini.", "Belohnt treue Stammkunden nach einer bestimmten Anzahl Bestellungen mit einem automatischen Rabattcode."),
      trigger: t("After X orders from same customer", "Aynı müşteriden X sipariş sonra", "Après X commandes du même client", "Tras X pedidos del mismo cliente", "Dopo X ordini dello stesso cliente", "Nach X Bestellungen desselben Kunden"),
      fields: [
        { key: "order_threshold", label: t("Number of orders", "Sipariş sayısı", "Nombre de commandes", "Número de pedidos", "Numero ordini", "Anzahl Bestellungen"), type: "number", suffix: suffixOrders, default: 3, min: 2, max: 20 },
        { key: "discount_pct", label: discount, type: "number", suffix: "%", default: 15, min: 5, max: 30 },
      ],
      stat_label: t("codes sent", "kod gönderildi", "codes envoyés", "códigos enviados", "codici inviati", "Codes versendet"),
      status: "coming_soon",
    },
    {
      id: "price_drop_alert",
      ...
      accentColor: "#06b6d4",
      accentBg: "#cffafe",
      title: t("Price drop alert", "Fiyat düşüşü uyarısı", "Alerte baisse de prix", "Alerta de bajada de precio", "Avviso calo prezzo", "Preisänderungs-Alarm"),
      desc: t("Informs customers on the wishlist when a saved product becomes cheaper.", "Kayıtlı ürün ucuzladığında istek listesindeki müşterilere bildirir.", "Informe les clients de la liste de souhaits quand un produit enregistré baisse de prix.", "Informa a clientes en la lista de deseos cuando un producto guardado baja de precio.", "Informa i clienti nella wishlist quando un prodotto salvato diventa più economico.", "Informiert Kunden auf der Merkliste, sobald ein gespeichertes Produkt günstiger wird."),
      trigger: t("Product price reduced", "Ürün fiyatı düşürüldü", "Prix produit réduit", "Precio de producto reducido", "Prezzo prodotto ridotto", "Produktpreis gesenkt"),
      fields: [],
      stat_label: t("alerts sent", "uyarı gönderildi", "alertes envoyées", "alertas enviadas", "avvisi inviati", "Alerts gesendet"),
      status: "coming_soon",
    },
  ];
  ...
  return {
    categories,
    automationDefs,
    comingSoon: t("Coming soon", "Yakında", "Bientôt", "Próximamente", "In arrivo", "Demnächst"),
    active: t("Active", "Aktif", "Actif", "Activo", "Attivo", "Aktiv"),
    ...
    pageSubtitle: t("Automated workflows that work for you around the clock.", "Günün her saati sizin için çalışan otomatik iş akışları.", "Workflows automatisés qui travaillent pour vous 24h/24.", "Flujos automatizados que trabajan para ti las 24 horas.", "Workflow automatizzati che lavorano per te 24 ore su 24.", "Automatisierte Workflows, die rund um die Uhr für dich arbeiten."),
    statActiveFlows: t("Active flows", "Aktif akışlar", "Flux actifs", "Flujos activos", "Flussi attivi", "Aktive Flows"),
    statTriggered: t("Triggered (total)", "Tetiklendi (toplam)", "Déclenchés (total)", "Activados (total)", "Attivati (totale)", "Ausgelöst (gesamt)"),
    statAvailable: t("Available automations", "Kullanılabilir otomasyonlar", "Automatisations disponibles", "Automatizaciones disponibles", "Automazioni disponibili", "Verfügbare Automationen"),
    savedSuccess: t("Saved ✓", "Kaydedildi ✓", "Enregistré ✓", "Guardado ✓", "Salvato ✓", "Gespeichert ✓"),
    loadError: t("Could not load data.", "Veriler yüklenemedi.", "Impossible de charger les données.", "No se pudieron cargar los datos.", "Impossibile caricare i dati.", "Daten konnten nicht geladen werden."),
    saveError: t("Error saving.", "Kaydetme hatası.", "Erreur lors de l'enregistrement.", "Error al guardar.", "Errore durante il salvataggio.", "Fehler beim Speichern."),
    howItWorksTitle: t("How do automations work?", "Otomasyonlar nasıl çalışır?", "Comment fonctionnent les automatisations ?", "¿Cómo funcionan las automatizaciones?", "Come funzionano le automazioni?", "Wie funktionieren Automationen?"),
    howItWorksBody: t("Configured rules are checked hourly and run automatically. Emails are sent via the SMTP connection configured under Settings → Email. Make sure SMTP is set up correctly so automations can trigger.", "Yapılandırılan kurallar saatlik kontrol edilir ve otomatik çalıştırılır. E-postalar Ayarlar → E-posta altında yapılandırılan SMTP bağlantısı üzerinden gönderilir. Otomasyonların tetiklenebilmesi için SMTP'nin doğru kurulduğundan emin olun.", "Les règles configurées sont vérifiées chaque heure et exécutées automatiquement. Les e-mails sont envoyés via la connexion SMTP configurée sous Paramètres → E-mail. Assurez-vous que SMTP est correctement configuré.", "Las reglas configuradas se comprueban cada hora y se ejecutan automáticamente. Los correos se envían vía SMTP en Configuración → Correo. Asegúrate de que SMTP esté configurado correctamente.", "Le regole configurate vengono controllate ogni ora ed eseguite automaticamente. Le email vengono inviate via SMTP in Impostazioni → Email. Assicurati che SMTP sia configurato correttamente.", "Die konfigurierten Regeln werden stündlich geprüft und automatisch ausgeführt. E-Mails werden über die unter Einstellungen → E-Mail konfigurierte SMTP-Verbindung versendet. Stelle sicher, dass SMTP korrekt eingerichtet ist, damit die Automationen ausgelöst werden können."),
