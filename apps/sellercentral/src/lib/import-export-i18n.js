...
export function getImportApiMessages(locale) {
  ...
  return {
    ...
    unknownBrandChild: (name) =>
      t(`Unknown brand (child row): "${name}". Register it under Content → Brands in Seller Central, then import again.`, `Bilinmeyen marka (child satır): "${name}". Seller Central'da Content → Brands üzerinden kaydedin, sonra yeniden içe aktarın.`, `Marque inconnue (ligne enfant) : « ${name} ». Enregistrez-la sous Content → Brands dans Seller Central, puis réimportez.`, `Marca desconocida (fila child): "${name}". Regístrela en Content → Brands en Seller Central y vuelva a importar.`, `Marca sconosciuta (riga child): "${name}". Registrala in Content → Brands su Seller Central, poi importa di nuovo.`, `Unbekannte Marke (child row): "${name}". Legen Sie sie unter Content → Brands in Seller Central an und importieren Sie erneut.`),
    ...
    unknownBrand: (name) =>
      t(`Unknown brand (not in system): "${name}". Register it under Content → Brands in Seller Central, then import again.`, `Bilinmeyen marka (sistemde yok): "${name}". Seller Central'da Content → Brands üzerinden kaydedin, sonra yeniden içe aktarın.`, `Marque inconnue (pas dans le système) : « ${name} ». Enregistrez-la sous Content → Brands dans Seller Central, puis réimportez.`, `Marca desconocida (no en el sistema): "${name}". Regístrela en Content → Brands en Seller Central y vuelva a importar.`, `Marca sconosciuta (non nel sistema): "${name}". Registrala in Content → Brands su Seller Central, poi importa di nuovo.`, `Unbekannte Marke (nicht im System): "${name}". Legen Sie sie unter Content → Brands in Seller Central an und importieren Sie erneut.`),
...
export function getImportExportCopy(locale) {
  ...
  return {
    ...
    importRulesList: [
      ...
      t("Eigenschaften: metafield_N_key / metafield_N_value dropdowns from Metaobjects. Custom titles/values are allowed; the product stays hidden in the shop until a superuser approves them.", "Eigenschaften: metafield_N_key / metafield_N_value Metaobjects açılır listeleri. Özel başlık/değer yazılabilir; superuser onaylayana kadar ürün shop’ta görünmez.", "Eigenschaften : listes Metaobjects. Saisie libre possible ; le produit reste masqué jusqu’à approbation superuser.", "Eigenschaften: desplegables de Metaobjects. Puede escribir valores propios; el producto no aparece hasta que un superusuario los apruebe.", "Eigenschaften: menu Metaobjects. Valori propri ammessi; il prodotto resta nascosto fino all'approvazione superuser.", "Eigenschaften: metafield_N_key / metafield_N_value mit Dropdowns aus Metaobjects. Eigene Titel/Werte erlaubt — Produkt bleibt im Shop unsichtbar, bis ein Superuser sie freigibt."),
    ...
    exportSubtitle: t("Choose one data type, pick columns, then download a clean spreadsheet.", "Bir veri türü seçin, sütunları işaretleyin, temiz bir tablo indirin.", "Choisissez un type, les colonnes, puis téléchargez un tableau propre.", "Elija un tipo, las columnas y descargue una tabla limpia.", "Scegli un tipo, le colonne e scarica un foglio pulito.", "Einen Datentyp wählen, Spalten ankreuzen und eine saubere Tabelle laden."),
    exportScope: t("1) Data type", "1) Veri türü", "1) Type de données", "1) Tipo de datos", "1) Tipo di dati", "1) Datentyp"),
    exportScopeHint: t("Select only one: products, orders or transactions.", "Yalnızca biri: ürünler, siparişler veya işlemler.", "Un seul choix : produits, commandes ou transactions.", "Solo uno: productos, pedidos o transacciones.", "Uno solo: prodotti, ordini o transazioni.", "Nur eines: Produkte, Bestellungen oder Transaktionen."),
    datasetProducts: t("Products", "Ürünler", "Produits", "Productos", "Prodotti", "Produkte"),
    datasetOrders: t("Orders", "Siparişler", "Commandes", "Pedidos", "Ordini", "Bestellungen"),
    datasetCustomers: t("Customers", "Müşteriler", "Clients", "Clientes", "Clienti", "Kunden"),
    datasetTransactions: t("Transactions", "İşlemler", "Transactions", "Transacciones", "Transazioni", "Transaktionen"),
    datasetRanking: t("Views / clicks", "Görüntülenme / tıklama", "Vues / clics", "Vistas / clics", "Visualizzazioni / clic", "Aufrufe / Klicks"),
    exportFilters: t("2) Filters", "2) Filtreler", "2) Filtres", "2) Filtros", "2) Filtri", "2) Filter"),
    filterSearch: t("Search (SKU, name, email…)", "Arama (SKU, ad, e-posta…)", "Recherche (SKU, nom, e-mail…)", "Búsqueda (SKU, nombre, email…)", "Cerca (SKU, nome, email…)", "Suche (SKU, Name, E-Mail…)"),
    filterStatus: t("Status (optional)", "Durum (isteğe bağlı)", "Statut (optionnel)", "Estado (opcional)", "Stato (opzionale)", "Status (optional)"),
    filterStatusAll: t("All statuses", "Tüm durumlar", "Tous les statuts", "Todos los estados", "Tutti gli stati", "Alle Status"),
    ...
    statusOpen: t("Open", "Açık", "Ouvert", "Abierto", "Aperto", "Offen"),
    statusProcessing: t("Processing", "İşleniyor", "En cours", "En proceso", "In elaborazione", "In Bearbeitung"),
    statusCompleted: t("Completed", "Tamamlandı", "Terminé", "Completado", "Completato", "Abgeschlossen"),
    statusCancelled: t("Cancelled", "İptal", "Annulé", "Cancelado", "Annullato", "Storniert"),
    includeAllSellers: t("Superuser: include all seller data", "Süper kullanıcı: tüm satıcı verileri dahil", "Superuser : inclure toutes les données vendeurs", "Superusuario: incluir todos los vendedores", "Superuser: includi tutti i venditori", "Superuser: alle Verkäuferdaten einschließen"),
    groupBySeller: t("Split into seller sheets in XLSX export", "XLSX dışa aktarmada satıcı sayfalarına ayır", "Séparer par vendeur dans l'export XLSX", "Separar por vendedor en exportación XLSX", "Separa per venditore nell'export XLSX", "Im XLSX nach Verkäufer aufteilen"),
    exportColumns: t("3) Columns", "3) Sütunlar", "3) Colonnes", "3) Columnas", "3) Colonne", "3) Spalten"),
    recommendedColumns: t("Recommended", "Önerilen", "Recommandé", "Recomendadas", "Consigliate", "Empfohlen"),
    selectAll: t("Select all", "Tümünü seç", "Tout sélectionner", "Seleccionar todo", "Seleziona tutto", "Alle auswählen"),
    clear: t("Clear", "Temizle", "Effacer", "Limpiar", "Cancella", "Leeren"),
    columnsSelected: (n) => t(`${n} columns selected`, `${n} sütun seçili`, `${n} colonnes sélectionnées`, `${n} columnas seleccionadas`, `${n} colonne selezionate`, `${n} Spalten ausgewählt`),
