  ...
  try {
    ...
    addSheet(
      ...
      [
        ...
        ["Erstellt am", stamp],
        ["Warenwert (Verkäufer-GMV, nicht Andertal-Umsatz)", eur(t.gross_sale_cents)],
        ["Versand (Kunde, vom Kunden gezahlt)", eur(t.shipping_cents)],
        ["Versand (Plattform, Andertal bezahlt — nicht an Verkäufer)", eur(t.label_cents)],
        ["Bestellwert (Ware + Kundenversand)", eur((t.gross_sale_cents || 0) + (t.shipping_cents || 0))],
        ["Vom Kunden gezahlt", eur(t.customer_paid_cents)],
        ["Von Bonuspunkten gezahlt (Andertal, Aufwand)", eur(t.bonus_funding_cents)],
        ["Provision netto", eur(t.commission_net_cents)],
        ["Provision USt", eur(t.commission_vat_cents)],
        ["Provision inkl. MwSt. (Andertal-Umsatz, steuerpflichtig)", eur((t.commission_net_cents || 0) + (t.commission_vat_cents || 0))],
        ["Kundenversand an Verkäufer ausgezahlt", eur(t.shipping_payout_cents)],
    ...
    addSheet(
      ...
      [
        ...
        "Warenwert", "Versand Kunde", "Versand Plattform", "Versand an Verkäufer ausgezahlt",
        "Vom Kunden gezahlt", "Von Bonuspunkten gezahlt",
        "Provision netto", "Provision USt", "Provision inkl. MwSt.",
        "Auszahlung", "Erstattung", "Bestellungen",
      ],
      sellers.map((s) => [
        ...
        s.status || "—",
        eur(s.gross_sale_cents),
        eur(s.shipping_cents),
        eur(s.label_cents),
        eur(s.shipping_payout_cents),
        eur(s.customer_paid_cents), eur(s.bonus_funding_cents),
        eur(s.commission_net_cents), eur(s.commission_vat_cents),
        eur((s.commission_net_cents || 0) + (s.commission_vat_cents || 0)),
        eur(s.seller_payout_cents),
    ...
    addSheet(
      wb,
      "OSS_Bestimmungsland",
      ["Zielland", "Bestellungen", "Warenwert (Bestellwert)", "Nettoumsatz (Ware)", "USt Ware", "USt-Satz %"],
      oss.map((row) => [
