...
29) **[Ürün gösterimi] Sellercentral → Shop ürün akışı incelemesi.** Onay/görünürlük filtrelemesini kod üzerinden doğruladım: onaylanmamış/reddedilmiş satıcıların ürünleri arama, kategori ve marka listelerinde tutarlı şekilde filtreleniyor — bu sağlam. İki bulgu:
- Bir ürün Sellercentral'da güncellendikten sonra shop'ta görünmesi **60 saniyeye kadar gecikebilir** (performans için kasıtlı önbellekleme — `Cache-Control: max-age=60`), ama Sellercentral'daki "Kaydet" sonrası satıcıya bu konuda hiçbir bilgi verilmiyor. Satıcı "kaydettim ama değişmedi" diye endişelenebilir.
- Çok yakın zamanda eklenen yeni "özel ürün rozeti" (custom badge) sistemi (`product-badges.js`, `CustomProductBadge.jsx`, `ProductImageBadges.jsx` — toplam 178 satır) henüz hiç gözden geçirilmedi/test edilmedi, potansiyel taze hata kaynağı.

30) **[Ciddi, "kusursuz" beklentisiyle çelişiyor] CSV/Excel toplu ürün içe aktarma fonksiyonunda gerçek hatalar bulundu.**
- Sayfa gerçek Excel (.xlsx) dosyası **kabul etmiyor**, sadece .csv — satıcı Excel'den doğrudan yükleyebileceğini düşünüp kafası karışabilir.
- El yapımı CSV ayrıştırıcı, tırnak içindeki satır sonlarını (örn. çok satırlı ürün açıklaması) **yanlış bölüyor** — veri bozulması riski taşıyor.
- **Şablonda "kategori" ve "marka" sütunları var ama gerçekte hiçbir işe yaramıyor** — girilen isimler kullanılmayan bir metadata alanına yazılıyor, ürün gerçek bir kategori/markaya bağlanmıyor. Satıcı "Elektronik" yazsa bile ürün o kategoride görünmeyecek. Bu, "kusursuz çalışmalı" beklentisiyle doğrudan çelişen, sessiz bir veri kaybı.
- SKU/EAN tekillik kontrolü yapılmıyor — ürün oluşturmanın diğer yollarında (tekil ürün ekleme) var olan bu kontrol, toplu içe aktarmada atlanmış; yinelenen EAN'lı ürünler oluşabilir.
- 500 satıra kadar her satır ayrı, sıralı bir HTTP isteğiyle işleniyor — büyük dosyalarda (örn. 500 ürün) çok yavaş olabilir, zaman aşımı riski taşır.
